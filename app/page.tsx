...
const PROXY_URL = "https://yahoo-proxy-seven.vercel.app/api/price";
...
function DateTimePicker24h({ value, onChange }: { value: string; onChange: (iso: string) => void }) {
  ...
  const update = (nd: string, nm: string, ny: string, nhh: string, nmm: string) => {
    if (nd && nm && ny && ny.length === 4) {
      const iso = `${ny}-${nm.padStart(2,"0")}-${nd.padStart(2,"0")}T${(nhh||"00").padStart(2,"0")}:${(nmm||"00").padStart(2,"0")}`;
      onChange(iso);
    }
  };

  const fieldStyle = { background:"#0f1117", border:"1px solid #4a5568", borderRadius:6, color:"#e2e8f0", fontSize:14, padding:"10px 6px", textAlign:"center" as const, boxSizing:"border-box" as const };

  return (
    <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}>
      <input type="number" min={1} max={31} value={d} placeholder="DD" onChange={e=>update(e.target.value,m,y,hh,mm)} style={{...fieldStyle,width:50}}/>
      <span style={{color:"#718096"}}>/</span>
      <input type="number" min={1} max={12} value={m} placeholder="MM" onChange={e=>update(d,e.target.value,y,hh,mm)} style={{...fieldStyle,width:50}}/>
      <span style={{color:"#718096"}}>/</span>
      <input type="number" min={2000} max={2100} value={y} placeholder="YYYY" onChange={e=>update(d,m,e.target.value,hh,mm)} style={{...fieldStyle,width:72}}/>
      <span style={{color:"#718096",marginLeft:4}}>เวลา</span>
      <input type="number" min={0} max={23} value={hh} placeholder="HH" onChange={e=>update(d,m,y,e.target.value,mm)} style={{...fieldStyle,width:46}}/>
      <span style={{color:"#718096"}}>:</span>
      <input type="number" min={0} max={59} value={mm} placeholder="mm" onChange={e=>update(d,m,y,hh,e.target.value)} style={{...fieldStyle,width:46}}/>
      <span style={{color:"#4a5568",fontSize:11}}>(24h)</span>
    </div>
  );
}
...
export default function App() {
  ...
  const copyForAnalysis = () => {
    const tv=holdings.reduce((s:number,h:any)=>s+h.shares*h.currentPrice,0); const tc=holdings.reduce((s:number,h:any)=>s+h.shares*h.avgCost,0); const pnl=tv-tc; const pnlPct=tc>0?pnl/tc*100:0;
    ...
    const movers=holdings.filter((h:any)=>h.changePct!=null&&Math.abs(h.changePct)>=3).sort((a:any,b:any)=>Math.abs(b.changePct)-Math.abs(a.changePct)).map((h:any)=>`${h.symbol} ${h.changePct>0?"+":""}${h.changePct}%`).join(", ");
    const losersText=buildLosersText(holdings,tv);
    const p=`คุณคือ Senior Portfolio Manager และ CFA Charterholder วันนี้คือ ${lastUpdate}\n\n## Investment Philosophy\n- Long-term buy & hold หลายปี\n- ขาดทุน ≠ ต้องขาย ถ้าพื้นฐานดียังถือต่อได้\n- เป้าหมาย: หาตัวที่ควรขายจริงๆ เท่านั้น\n\n## กฎ\n1. ห้ามแนะนำขายแค่เพราะ P&L ติดลบ\n2. ขายได้เฉพาะ: business model พัง, moat หายไป, management แย่, valuation bubble\n3. ถ้าไม่มีเหตุผลพื้นฐาน → "ถือต่อ"\n\n## รูปแบบ\n📊 PORTFOLIO OVERVIEW\n🔴 ขายจริงๆ (เหตุผลพื้นฐาน)\n⚠️ เฝ้าระวัง\n✅ TOP LOSERS — วิเคราะห์พื้นฐาน\n\n---\nPORTFOLIO "${currentPortName}" (${holdings.length} positions | $${tv.toFixed(0)} | P&L ${pnl>=0?"+":""}${pnlPct.toFixed(2)}%)\n${movers?`\n🚨 ผิดปกติวันนี้: ${movers}\n`:""}\n${rows}${losersText}\n\n---\nราคาอัพเดท ${lastUpdate} | วิเคราะห์เป็นภาษาไทย`;
    copyToClipboard(p); msg("Copy prompt แล้ว ✓");
  };

  const copyMoversAnalysis = () => {
    const movers=holdings.filter((h:any)=>h.changePct!=null&&Math.abs(h.changePct)>=3).sort((a:any,b:any)=>Math.abs(b.changePct)-Math.abs(a.changePct));
    if (!movers.length) { msg("ไม่มีหุ้นผิดปกติวันนี้ ⚡"); return; }
    const tv=holdings.reduce((s:number,h:any)=>s+h.shares*h.currentPrice,0); const lastUpdate=new Date().toLocaleDateString("th-TH",{year:"numeric",month:"long",day:"numeric"});
  ...
  const copyAllocationAnalysis = () => {
    const tv=holdings.reduce((s:number,h:any)=>s+h.shares*h.currentPrice,0); const tc=holdings.reduce((s:number,h:any)=>s+h.shares*h.avgCost,0); const lastUpdate=new Date().toLocaleDateString("th-TH",{year:"numeric",month:"long",day:"numeric"});
  ...
  const moversCount=holdings.filter((h:any)=>h.changePct!=null&&Math.abs(h.changePct)>=3).length;
