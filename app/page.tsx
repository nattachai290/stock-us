...
export default function App() {
  ...
  const copyBuyDecision = () => {
    ...
    // under-target semis look fine one at a time and awful together, so Claude gets the sector
    // weights and the names already at target — as background only, never as buy candidates.
    const secVal:Record<string,number>={};
    activeHoldings.forEach((h:any)=>{ const k=h.sector||"ไม่ระบุ"; secVal[k]=(secVal[k]||0)+h.shares*h.currentPrice; });
